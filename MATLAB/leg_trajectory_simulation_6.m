...
%% Smooth movement of simultaneous 2&3joint movement
theta1 =[60,60,60,60,60,60,55,50,45,40,35,40,45,50,55,60];  %{first link}
theta2 = [-120,-110,-100,-90,-80,-70,-70,-70,-70,-70,-70,-80,-90,-100,-110,-120];  %{Second link}
theta3 = [-30,-35,-40,-45,-50,-55,-55,-55,-55,-55,-55,-50,-45,-40,-35,-30];  %{Third link} 
gamma = theta1+theta2+theta3;
 %% Smooth movement of simultaneous ALL joints movement % impossible because it would just slide on the window
% theta1 =[60,55,50,45,40,35,35,35,35,35,35,40,45,50,55,60];   %{first link}
% theta2 = [-120,-110,-100,-90,-80,-70,-70,-70,-70,-70,-70,-80,-90,-100,-110,-120];  %{Second link}
% theta3 = [-30,-35,-40,-45,-50,-55,-55,-55,-55,-55,-55,-50,-45,-40,-35,-30];  %{Third link} 
...
%     'LineStyle',':')    %drawing a cricle for centre at (2,0.5)
    plot(PP1x, (PP1y+.5), '--<','color',[0.4660 0.6740 0.1880])
    plot(PP2x, (PP2y+.5), '--<','color',[0.8500 0.3250 0.0980])
    plot(PP3x, (PP3y+.5), '--<','color',[0.4940 0.1840 0.5560])
...
videofile = VideoWriter('Forward_Kinematics_run_7.avi','Uncompressed AVI');
%{to open the file}
open(videofile)   
...
plot(theta1,'or')
xlabel('Time [sec]')
ylabel('Leg Position of joint 3  [deg]')
title('Position of All 3 joints')

%We are using the Cubic formula for path planning as we want different
%torque values for different cases
